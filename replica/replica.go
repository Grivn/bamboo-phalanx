...
func (r *Replica) HandleCommand(command protos.Command) {
	r.startSignal()
	log.Debugf("[%v] received a command-message from %v", r.ID(), command.Author)
	r.eventChan <- command
}
...
func (r *Replica) handleTxn(m message.Transaction) {
	...
	tx := types2.GenerateTransaction(payload)
